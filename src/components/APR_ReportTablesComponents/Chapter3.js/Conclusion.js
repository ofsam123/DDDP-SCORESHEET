...
function Conclusion({ year, district, assessmentStatus, assessmentStatus1 }) {
  const { user } = useAuth();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (existingCommentId) {
        ...
        message.success("Conclussion updated successfully");
        setEditing(false);
      } else {
        ...
        message.success("Conclussion added successfully");
        setEditing(false);
      }
    } catch (error) {
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div style={{ padding: "20px" }}>
              <p>No Conclussion available for this district.</p>
            </div>
          </div>
        </div>
      </div>
