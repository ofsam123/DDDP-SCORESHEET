...
function Introduction({ year, district, assessmentStatus }) {
  const { user } = useAuth();
      ...
      try {
        ...
        const filteredComments = response.data.filter(
          (comment) =>
            ...
            comment.userRole === "APRIntroduction"
        );
  ...
  const handleSave = async () => {
    ...
    if (!isQualityAssurance) {
      message.error("Only APR USER can edit the Introduction");
      return;
    }
    ...
    try {
      ...
      message.error(`Failed to save Introduction: ${error.response?.data?.message || error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderCommentList = () => {
    ...
    if (!comment) {
      return <p>No Introduction available for this district.</p>;
    }
    return (
      <div
        style={{
