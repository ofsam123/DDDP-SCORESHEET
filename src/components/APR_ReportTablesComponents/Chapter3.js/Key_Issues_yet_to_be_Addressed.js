...
function KIYTBA({ year, district, assessmentStatus }) {
  ...
  useEffect(() => {
    const fetchComment = async () => {
      ...
      try {
        ...
        console.error("Failed to fetch Key Issues yet to be Addressed comment:", error);
        // message.error("Failed to fetch Key Issues yet to be Addressed");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    ...
    if (!isQualityAssurance) {
      message.error("Only APR USER can edit the Key Issues yet to be Addressed");
      return;
    }
    ...
    try {
      ...
      if (existingCommentId) {
        ...
        message.success("Key Issues yet to be Addressed updated successfully");
        setEditing(false);
      } else {
        const response = await instance.post("comments", payload);
        setComments([...comments, response.data]);
        setExistingCommentId(response.data.id);
        message.success("Key Issues yet to be Addressed added successfully");
        setEditing(false);
      }
    } catch (error) {
      console.error("Failed to save Key Issues yet to be Addressed:", {
        message: error.message,
        response: error.response?.data,
        status: error.response?.status,
      });
      message.error(`Failed to save Key Issues yet to be Addressed: ${error.response?.data?.message || error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderCommentList = () => {
    ...
    if (!comment) {
      return <p>No Key Issues yet to be Addressed available for this district.</p>;
    }
    return (
      <div
        style={{
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div style={{ padding: "20px" }}>
              <p>No Key Issues yet to be Addressed available for this district.</p>
            </div>
          </div>
        </div>
      </div>
              ...
              <div style={{ display: editing || !existingCommentId ? "block" : "none" }}>
                ...
                >
                  <span style={{ color: "white", fontSize: "14px", fontWeight: "bold" }}>
                    Save Key Issues yet to be Addressed
                  </span>
                </Button>
              </div>
            )}
