...
function DistrictAnalytics() {
  ...
  useEffect(() => {
    if (selectedDistrict) {
      fetchTotals(selectedDistrict.value);
  ...
  async function fetchInvitationData() {
      try {
        ...
        const response = await axios.get(
          `/analytics.json?dimension=dx:yhzMdqZp0Qh;aeKyGvo5OIp&dimension=ou:${selectedDistrict.value}&filter=pe:${year}-01-01;${year}-12-31`
        );
  ...
  async function fetchMeetingReportData() {
    ...
    try {
      ...
      const analyticsResponse = await axios.get(
        `/analytics.json?dimension=dx:aeKyGvo5OIp;kaxkMvOIXwW&dimension=ou:${selectedDistrict.value}&filter=pe:${year}-01-01;${year}-12-31`
      );
      ...
      rows.forEach(([dataElement, orgUnit, value]) => {
        if (orgUnit === selectedDistrict.value) {
  ...
  async function fetchActionPlanReportData() {
    ...
    try {
      ...
      const analyticsResponse = await axios.get(
        `/analytics.json?dimension=dx:L2oTOp0EA1A;csukj9I0QMB&dimension=ou:${selectedDistrict.value}&filter=pe:${year}-01-01;${year}-12-31`
      );
  ...
  async function fetchMeetingChartData() {
    try {
      const year = selectedYear.value;
      const response = await axios.get(
        `/analytics.json?dimension=dx:aeKyGvo5OIp;BEUJdCeTGIE;M86dDnKObvx;cz086QtLaoW;wGeWq6JhDQA&dimension=ou:${selectedDistrict.value}&filter=pe:${year}-01-01;${year}-12-31`
      );
      ...
      rows.forEach(([dataElement, orgUnit, value]) => {
        if (counts.hasOwnProperty(dataElement) && orgUnit === selectedDistrict.value) {
          counts[dataElement] += parseFloat(value) || 0;
  ...
  async function fetchMinuteInvitationData() {
    try {
      const year = selectedYear.value;
      const response = await axios.get(
        `/analytics.json?dimension=dx:RC8Fu7TLb7l;RsgxfezgTyr&dimension=ou:${selectedDistrict.value}&filter=pe:${year}-01-01;${year}-12-31`
      );
      // console.log("Minute Invitation Chart Analytics Response:", response.data);
      const rows = response.data.rows || [];
  ...
  async function pullTrackerInstance(districtId) {
    try {
      const year = selectedYear.value;
      await fetchTotals(districtId); // Fetch totals for the selected district
      ...
      <div className="page-content">
        ...
        <div className="page-header">
          <ol className="breadcrumb">
            ...
            <li className="breadcrumb-item active">District Analytics Dashboard</li>
          </ol>
        </div>
