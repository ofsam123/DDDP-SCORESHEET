...
function FoodVendors({
    ...
    function getFoodVendors() {
        axios
            .get(`/tracker/trackedEntities?orgUnit=${district}&program=abiQOocP8YA`)
            .then(result => {
                if (result.data.instances.length > 0) {

                    axios
                        .get(`/tracker/events?program=abiQOocP8YA&orgUnit=${district}&startDate=${year}-01-01&endDate=${year}-12-31`)
                        .then(resp => {
