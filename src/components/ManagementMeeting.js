...
const ManagementMeeting = forwardRef(({ data, year, columns, districtId, hideComment }, ref) => {
  ...
  useEffect(() => {
    const fetchEndpointData = async () => {
      ...
      try {
        ...
        } else if (!relevantComment.dddpData?.tables?.managementMeeting) {
          setError("No managementMeeting data found in assessment_start_DAPT comment");
          setEndpointData(null);
        } else {
          const mgmtData = relevantComment.dddpData.tables.managementMeeting;
          console.log("Fetched managementMeeting:", mgmtData);
          setEndpointData({
            ...mgmtData,
            data: Array.isArray(mgmtData.data) ? mgmtData.data : [],
            fulfillment: mgmtData.fulfillment || "Not Fulfilled",
          });
          setError(null);
        }
        ...
        const filteredComments = response.data.filter(
          (comment) =>
            comment.tableCommented === `c2.0-2.3-${year}` &&
  ...
  // Transform data to handle React element objects (e.g., docs, invitation, recommendation)
  const transformMeetings = (meetings) => {
    ...
    return meetings.map((meeting) => {
      ...
      if (!meeting.docs && !meeting.invitation && !meeting.recommendation) {
      ...
      return {
        ...
        invitation: meeting.invitation && typeof meeting.invitation === "object" && meeting.invitation?.props?.href ? (
          <a
            ...
            className="px-2 text-primary fw-bold text-decoration-underline"
          >
            {meeting.invitation.props.children || "View Document"}
          </a>
        ) : typeof meeting.invitation === "string" ? (
          meeting.invitation
        ) : (
          "N/A"
        ),
  ...
  return (
    <Spin spinning={loading} tip="Loading Management Meetings data...">
      <Comment
      ...
      >
        {({ renderCommentInput }) => (
          <>
            <Title level={3}>
              CI 2.0 Functionality of Statutory & Mandatory Organs of the Assembly - 2.3 Management Meetings
            </Title>
            <Title level={4} style={{ marginTop: "10px" }}>Assessment Guide/Requirement</Title>
            <Content>
              From the DCD, obtain minutes of Management meetings for <strong>{year}</strong>.
              <br /><br />
              <ol>
                <li type="i">
                  If Management Meetings were held, at least quarterly, and duly attended by at least 8 out of 11 for
                  District Assemblies; 10 out of 13 for Municipal Assemblies; and 13 out of 16 for Metropolitan Assemblies
                  of HoDs in <strong>{year}</strong>.
                </li>
              </ol>
            ...
            {(endpointData?.data?.length > 0 || data?.data?.length > 0) ? (
              ...
              <Text>No Management Meetings data available</Text>
            )}
