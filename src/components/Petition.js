...
function Petition({ year, districtId,assessmentStatus }) {
  const { user } = useAuth();
  ...
  const currentUserRole = user?.user?.userRoles?.find(
    (role) => role.name === "DPAT QUALITY ASSURANCE" || role.name === "DPAT TECHNICAL TEAM" || role.name === "DPAT DISTRICT USERS" 
  )?.name || "";
  ...
  const isPetition= currentUserRole === "DPAT DISTRICT USERS";
    ...
    const fetchComment = async () => {
      ...
      try {
        ...
        const userComment = filteredComments.find(
          (comment) => comment.userRole === "DDUSER" && comment.username === currentUsername
        );
        if (userComment) {
  ...
  const handleSave = async () => {
    if (!editorContent.trim()) {
      message.error("Text Field cannot be empty");
      return;
    }
    ...
    if (!isPetition) {
      message.error("Only DPAT DISTRICT USERS can comment here");
      return;
    }
    ...
    if (existingComment && existingComment.username !== currentUsername && !existingCommentId) {
      message.error("Only one DPAT DISTRICT user can comment for this district.");
      return;
    }
    ...
    try {
      ...
      if (existingCommentId) {
        ...
        message.success("Petition  updated successfully");
        setEditing(false);
      } else {
        ...
        message.success("Petition  added successfully");
        setEditing(false);
      }
    } catch (error) {
      console.error("Failed to save petition :", {
        message: error.message,
        response: error.response?.data,
        status: error.response?.status,
      });
      message.error(`Failed to save petition : ${error.response?.data?.message || error.message}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        >
          <div style={{ display: "flex", marginBottom: "10px" }}>
            {comment.username === currentUsername && isPetition && assessmentStatus && (
            <Col>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", width: "800px" }}>
                ...
                <h4 style={{ margin: 0, fontSize: "13px" }}>
                  {comment.fullName} (DPAT DISTRICT USERS)
      ...
      {isPetition && canComment() && (
        ...
        <div style={{ display: editing || !existingCommentId ? "block" : "none" }}>
          ...
          <Button
            ...
            loading={loading}
          >
            <span style={{ color: "white", fontSize: "14px", fontWeight: "bold" }}>SEND PETITION</span>
