...
function ShelterTransactionalHousing({
    ...
    function getResidentialHomeCentre() {
        axios
            .get(`/tracker/trackedEntities?orgUnit=${district}&program=F7wJn9wdwcQ`)
            .then(result => {
                if (result.data.instances.length > 0) {

                    axios
                        .get(`/tracker/events?program=F7wJn9wdwcQ&orgUnit=${district}&startDate=${year}-01-01&endDate=${year}-12-31`)
                        .then(resp => {
