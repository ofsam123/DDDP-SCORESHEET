...
export const getOperationalHealthFacilityPayload = (data, orgUnit, program, trackedEntityType) => {
    ...
    data.forEach(row => {
        const tempDataSet = {
            ...
            trackedEntityType,
            attributes: [
                { attribute: "wca7mlI2exE", value: row[7] || "" }, // Name
                { attribute: "pF64ANEoPYx", value: row[8] || "" }, // Location
                { attribute: "KeSsfvs1awo", value: row[9] || "" }, // Owner/leader
                { attribute: "vIkEALDVio6", value: row[10] || "" }, // Ownersip Type

                { attribute: "IztkGQj5EqM", value: row[6] || "" }, // Unique id
                { attribute: "Xpj7aFWa3ii", value: row[11] || "" }, // Health FacilityType
                { attribute: "SKeHl5XTZVG", value: row[12] || "" }, // Email Address
                { attribute: "vUVEUS3VXbF", value: row[13] || "" }, // Postal Address
                { attribute: "cPGCeyMceZl", value: row[14] || "" }, // Site Coordinates
                { attribute: "YqP6CROSeSq", value: row[15] || "" }, // Digital Postal Address
                { attribute: "HgemfhhsXBc", value: row[16] || "" }, // Mobile Number
                { attribute: "yqvSryblNL4", value: row[17] || "" }, // Construction Start Date
                { attribute: "wSuCvQcwby6", value: row[18] || "" }, // Construction End Date
            ],
            enrollments: [
                {
                    program,
    ...
    data.forEach(row => {
        const tempDataSet = {
            ...
            attributes: [
                ...
                { attribute: "d4vGACWAj7R", value: row[5] || "" }, // Name of sport facility

                { attribute: "NU2g62mWHJQ", value: row[7] || "" }, // facility ownership type
                { attribute: "yqvSryblNL4", value: row[8] || "" }, // Construction Start Date
                { attribute: "wSuCvQcwby6", value: row[9] || "" }, // Construction End Date
                { attribute: "jBaDMK6rqCA", value: row[10] || "" }, // Number of direct beneficiary
                { attribute: "pF64ANEoPYx", value: row[11] || "" }, // Location
                { attribute: "fGfO0hrRRgq", value: row[12] || "" }, // Description
                { attribute: "EjkjhZFlslh", value: row[15] || "" }, // Purpose of sport facility
                { attribute: "EzZnJRWxDdN", value: row[16] || "" }, // Specify purpose of sport facility
    ...
    data.forEach(row => {
        const tempDataSet = {
            ...
            attributes: [
                ...
                { attribute: "Hq8it9qapWz", value: row[6] || "" }, // Service Provider
                { attribute: "ttyKWvSBAZH", value: row[8] || "" }, // Name of Business 
                { attribute: "XDjgAYmON2o", value: row[9] || "" }, // Address location
                { attribute: "SKeHl5XTZVG", value: row[10] || "" }, // Period of contract
                { attribute: "tb0nGRwpQEw", value: row[11] || "" }, // Start date

                { attribute: "YqP6CROSeSq", value: row[12] || "" }, // Digital postal address
                { attribute: "pF64ANEoPYx", value: row[13] || "" }, // Location
                { attribute: "sGe7MGw4BVN", value: row[14] || "" }, // Phone
                { attribute: "SKeHl5XTZVG", value: row[15] || "" }, // Email address
...
export const getSchoolProfilePayload = (data, orgUnit, program, trackedEntityType) => {
    ...
    data.forEach(row => {
        const tempDataSet = {
            ...
            trackedEntityType,
            attributes: [
                { attribute: "UJBblniqPOI", value: row[12] || "" }, // Community

                { attribute: "IztkGQj5EqM", value: row[6] || "" }, // Unique id
                { attribute: "aNfKUv5kVo7", value: row[7] || "" }, // Name of School
                { attribute: "PX7m8i2KVgT", value: row[9] || "" }, // Is school faith based
                { attribute: "AlVMRjjWuVo", value: row[13] || "" }, // Established date
                { attribute: "YqP6CROSeSq", value: row[14] || "" }, // Digital postal address
                { attribute: "cPGCeyMceZl", value: row[15] || "" }, // Site coordinates
                { attribute: "wO2BqNRuILL", value: row[23] || "" }, // Site photo
            ],
            enrollments: [
                {
                    program,
...
export const getMeetingsPayload = (data, orgUnit, program, trackedEntityType) => {
    ...
    data.forEach(row => {
        const tempDataSet = {
            ...
            trackedEntityType,
            attributes: [
                { attribute: "eOsejnvfYiV", value: row[6] || "" }, // Invitation letter date
                { attribute: "Mm3f9fbAT8r", value: row[7] || "" }, // Invitation letter reference number
                { attribute: "rSKzHSBlOyB", value: row[9] || "" }, // Who signed the invitation letter
                { attribute: "Ub0V9Z06aBc", value: row[10] || "" }, // Meeting date
                { attribute: "kghpIZgHFsT", value: row[12] || "" }, // Meeting type
                { attribute: "b7uAJaX9obN", value: row[17] || "" }, // Meeting title
                { attribute: "HmdJqjjAZZ4", value: row[19] || "" }, // Meeting venue
                { attribute: "Br9IEOZyKPW", value: row[23] || "" }, // Number of desicions
                { attribute: "U2RjiIApoYu", value: row[26] || "" }, // Number of Attendance Male
                { attribute: "iHz18pXkmMd", value: row[27] || "" }, // Number of Attendance Female


                { attribute: "AiPyh2A2DZ8", value: row[8] || "" }, // Means of invitation letter distribution
                { attribute: "unVPW9dCS5B", value: row[11] || "" }, // Minute file number
                { attribute: "j0kNoCtC8pj", value: row[13] || "" }, // General assembly meeting types
                { attribute: "hfCSE8B2L9p", value: row[14] || "" }, // Statutory committee types
                { attribute: "sI8Kj3pfa6k", value: row[15] || "" }, // Name of sub structure
                { attribute: "f1V1vDibPCS", value: row[16] || "" }, // Other (specify)
                { attribute: "mH8uydlfv03", value: row[18] || "" }, // Meeting agenda
                { attribute: "uRdPt6OIxvU", value: row[24] || "" }, // Number of cpmplaints
                { attribute: "DGDc7z1ESlb", value: row[25] || "" }, // Number of recommendations made
                { attribute: "SdtJSE3jJkV", value: row[28] || "" }, // Number of participant HoDs Male
                { attribute: "RFSUywjaDRi", value: row[29] || "" }, // Number of participant HoDs Female
                { attribute: "Fup3ibG5EJc", value: row[31] || "" }, // Was the annual action plan approved
                { attribute: "xh6NIEHa8xg", value: row[32] || "" }, // AAP approval date
            ],
            enrollments: [
                {
                    program,
